		...
		public override void RegisterCollision(Collidable p_Other)
		{
			...
			if (!(p_Other is Shot))
			{
				...
				if (m_TailState == TailState.Throwing)
				{
					...
					// Is this really necessary? or, is this the right place to put it? Also, why were you using the texture height and width?
					...
					if (Math.Abs(Math.Abs(m_EnemyCaught.Center.Y) - Math.Abs(p_Other.Center.Y)) < Math.Abs(Math.Abs(m_EnemyCaught.Center.X) - Math.Abs(p_Other.Center.X)))
						if (m_EnemyCaught.Center.X < p_Other.Center.X)
							tempVect.X = p_Other.Position.X - (m_EnemyCaught.Width + 20);
						else
							tempVect.X = p_Other.Position.X + (p_Other.Width + 20);

					if (Math.Abs(Math.Abs(m_EnemyCaught.Center.Y) - Math.Abs(p_Other.Center.Y)) > Math.Abs(Math.Abs(m_EnemyCaught.Center.X) - Math.Abs(p_Other.Center.X)))
						if (m_EnemyCaught.Center.Y < p_Other.Center.Y)
							tempVect.Y = p_Other.Position.Y - (m_EnemyCaught.Height + 20);
						else
							tempVect.Y = p_Other.Position.Y + (p_Other.Height + 20);
